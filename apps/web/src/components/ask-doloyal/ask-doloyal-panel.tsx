...
import { Button, Textarea, Select, SelectTrigger, SelectValue, SelectContent, SelectItem, cn } from "@doloyal/ui";
...
/**
 * Markdown rendering is code-split: the renderer (react-markdown +
 * remark-gfm) is only fetched when a message actually needs it. Until the
 * chunk arrives the raw text renders with identical bubble styling.
 */
const MarkdownBody = nextDynamic(() => import("./markdown-body"), {
  ssr: false,
  loading: () => null,
});
...
export function AskDoloyalPanel() {
  const router = useRouter();
  const { isOpen, close, refreshUnread } = useAskDoloyal();
  ...
  // Keep this component mounted after first open (so in-flight state
  // survives close/reopen exactly as before) but render nothing while the
  // panel is closed.
