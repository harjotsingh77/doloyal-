  ...
  getDashboardOverview: (params?: { days?: string | number; from?: string; to?: string }): DashboardOverview => {
    ...
    return {
      ...
      kpis: {
        ...
        outstandingPoints: 128400,
        walletHolders: 842,
        pointsIssued30d: Math.round(18400 * (numDays / 30)),
        redemptionRatePct: 24.5,
        campaignRevenue: Math.round(48200 * (numDays / 30)),
        campaignCustomers: Math.round(86 * (numDays / 30)),
        campaignReached: Math.round(1240 * (numDays / 30)),
        campaignsSent: Math.max(1, Math.round(4 * (numDays / 30))),
