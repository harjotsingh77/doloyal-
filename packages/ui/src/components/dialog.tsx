...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ...
    className={cn(
      ...
      "data-[state=open]:animate-[lf-fade-in_0.2s_ease] data-[state=closed]:opacity-0",
      className,
    )}
    {...props}
  />
...
>(({ className, children, hideClose, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      {!hideClose ? (
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-md p-1 text-[rgb(var(--color-muted-foreground))] opacity-70 transition-opacity hover:opacity-100 focus:outline-none">
          <X className="h-4 w-4" />
